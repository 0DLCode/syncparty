...
function nextRoomTimecode() {
  return new Promise((resolve, reject) => {
    const socket = new WebSocket(`ws://${window.location.host.toString().split(':').shift()}:2300/`);
  ...
  if (userId) {
    ...
    fetchAll().then(() => {
      ...
      if (localRoom.host.uuid === localUser.uuid) {
        // HOST
        const hostSocket = new WebSocket(`ws://${window.location.host.toString().split()}:2300/`);
